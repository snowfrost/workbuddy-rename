...
- **非破坏性铁律（量化）**：备份先行 → 每批 ≤10 个文件逐批核对 → 回收站优先 → 出错即停

## 空间卫生与季度深检（融合自马厩管理法）

2026-09-20 融合了 [马厩管理法 majiu-management](https://github.com/majiabin2020/majiu-management)（MIT）的三项方法论，
细则见 `references/space-hygiene.md`：

| 融合项 | 解决什么 |
|---|---|
| **草稿三分法** | 清 `04_temp/` 的判据：配方型（脚本/中间稿）可清但要先留「重生成指引」、资产型（AI 产物/当天数据）必须保留、依赖型（node_modules/venv）确认清单文件后可删。原则：**记忆存配方、磁盘存资产** |
| **非破坏性铁律量化** | 原来的红线只是定性，现在补上「备份先行 → 每批 ≤10 → 回收站优先 → 出错即停」 |
| **季度只读深检** | 与每日 9:30 的**写操作**归档互补：磁盘大户 / 冷文件 / 烂尾任务 / 记忆健康 → **只出报告、等拍板**，未拍板不动任何文件 |

> **两个 skill 的分工**：马厩法管**空间内**的日常纪律（命名、收尾、清废），本 skill 管**空间之间**的结构运维（分类、移动后修复引用）。
> 马厩法**不含**路径引用修复 —— 它的「梳理优化」只到移动 + 备份 + 回收站，
> 拿它直接整理 WorkBuddy 工作空间，会得到「点开空白 + 产物卡片打不开」。
> 因此**不建议把它装成第二个 skill**（触发词会撞车），而是把方法论吸收进来。
